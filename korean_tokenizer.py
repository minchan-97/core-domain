...
def clean_corpus(text: str) -> str:
    ...
    for line in lines:
        ...
        if re.search(r'0{4,}|\d{6,}', s): continue
        if '￭' in s or '◆' in s or '◇' in s: continue
        if re.search(r'\d{3,}[가-힣]{1,3}\d{3,}', s): continue
        ko_en = len(re.findall(r'[가-힣a-zA-Z]', s))
        total = len(s.replace(' ',''))
        if total > 0 and ko_en/total < 0.4: continue
        if re.match(r'^[\d\s]{8,}', s): continue
        words = s.split()
        if words and sum(len(w) for w in words)/len(words) > 15: continue
        key = re.sub(r'\s+','',s)[:40]
